...
class OODDetector:
    ...
    def detect_tensor(self, mask_prob: torch.Tensor) -> Dict[str, Any]:
        ...
        if p.numel() == 0:
            return {"ood_score": 0.0, "is_ood": False, "confidence": 1.0}
        ...
        score = float(score_t.item())
        confidence = float(torch.clamp(1.0 - score_t, min=0.0).item())
        return {
            "ood_score": score,
            "is_ood": bool(score > self.threshold),
...
class TTAPredictor:
    ...
    def __init__(
        ...
        if use_fast_mode:
            ...
            # Full mode (speed-optimized): elastic deform is opt-in via env or explicit augmentations.
            include_elastic = os.getenv("MEDSAM_TTA_INCLUDE_ELASTIC", "0").strip().lower() in {"1", "true", "yes", "y", "on"}
            base_augs = [
                "none",
                "hflip",
                "vflip",
            ...
            if include_elastic:
                base_augs.append("elastic_deform")
    ...
    def _build_tta_inputs_batch(
        ...
        device: str,
    ) -> Tuple[Dict[str, torch.Tensor], List[Tuple[str, int]], List[Tuple[int, int]], List[int]]:
        """
        ...
        aug_info_list: List[Tuple[str, int]] = []
        ...
        for sample_idx, (image, bbox) in enumerate(zip(images, bboxes)):
            ...
            for aug_name in self.augmentations:
                ...
                aug_info_list.append((aug_name, sample_idx))
                aug_count += 1
        ...
        if target_batch > total_augs:
            ...
            aug_info_list.extend([aug_info_list[-1]] * pad_count)
        ...
        return inputs, aug_info_list, output_sizes_list, true_aug_counts
    ...
    def _fuse_predictions(
        ...
        t0 = time.perf_counter() if profiler is not None and profiler.enabled else 0.0
        stacked_t = preds
        uncertainties_t = uncertainties.to(device=stacked_t.device, dtype=stacked_t.dtype)
        ...
        if self.fusion_mode == "mean":
            ...
            weights = torch.softmax(1.0 - uncertainties_t, dim=0)
    ...
    def predict_batch(
        ...
        inputs, aug_info_list, output_sizes_list, true_aug_counts = self._build_tta_inputs_batch(
            processor=processor,
            images=images,
            bboxes=bboxes,
            device=device,
        ...
        for sample_idx in range(n_samples):
            # Get augmentation indices for this sample
            sample_aug_indices = []
            for i, (aug_name, s_idx) in enumerate(aug_info_list):
                if s_idx == sample_idx:
                    sample_aug_indices.append(i)
            
            true_aug_count = true_aug_counts[sample_idx]
            sample_aug_indices = sample_aug_indices[:true_aug_count]
            
            # Extract predictions for this sample
            sample_preds = [pred_batch_t[i] for i in sample_aug_indices]
            aug_names = [aug_info_list[i][0] for i in sample_aug_indices]
            
            # Deaugment predictions
            t_deaug = time.perf_counter() if profiler is not None and profiler.enabled else 0.0
            preds_t = [
                self._deaugment_mask_tensor(pred, aug_name)
                for pred, aug_name in zip(sample_preds, aug_names)
            ]
            stacked_t = torch.stack(preds_t, dim=0)
            if profiler is not None and profiler.enabled:
                profiler.record_duration("tta.deaugment", time.perf_counter() - t_deaug)

            # Compute uncertainties
            prob_t = stacked_t.clamp(1e-6, 1.0 - 1e-6)
            entropy_t = -(prob_t * prob_t.log() + (1.0 - prob_t) * (1.0 - prob_t).log())
            uncertainties_t = entropy_t.mean(dim=(1, 2)).to(torch.float32)
...
def _predict_baseline_batch_tensor(
    *,
    ...
    for i, (sample_name, bbox) in enumerate(zip(sample_names, bboxes)):
        ...
        pred_t = torch.from_numpy(cached).to(device=device, dtype=torch.uint8, non_blocking=(device == "cuda"))
        pred_masks_t[i] = pred_t
        prob_for_ood_t[i] = pred_t.to(torch.float32)
    ...
    if miss_indices:
        ...
        for local_idx, global_idx in enumerate(miss_indices):
            ...
            prob_for_ood_t[global_idx] = pred_t.to(torch.float32)
            if pred_cache is not None:
                t_put = time.perf_counter()
                pred_cache.put(miss_keys[local_idx], pred_t.detach().cpu().numpy())
                if profiler is not None and profiler.enabled:
...
def evaluate_dataset(
    ...
    default_eval_workers = "8" if device == "cuda" else "0"
    ...
    for batch_samples in tqdm(iterable, total=total, desc=f"Evaluating {dataset_name}"):
        ...
        for i in range(len(batch_samples)):
            ...
            if use_ood and ood_detector is not None:
                ood = ood_detector.detect_tensor(prob_for_ood_t[i])
                ood_score = float(ood["ood_score"])
                is_ood = bool(ood["is_ood"])
                confidence = float(ood["confidence"])
                ood_scores.append(ood_score)
            ood_times.append(time.perf_counter() - t_ood)

            t_metric = time.perf_counter()
            m = compute_metrics_tensor(pred_masks_t[i], gt_masks_t[i])
            for k in metrics_keys:
                metrics_store[k].append(float(m[k]))
            metrics_times.append(time.perf_counter() - t_metric)
            ...
            post_times.append(time.perf_counter() - t_post)
            if profiler is not None and profiler.enabled:
